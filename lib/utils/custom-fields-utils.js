...
import { validateFieldData } from './field-validation-utils.js'
...
export async function mapFieldValuesToItem(fieldValues, fieldTemplates) {
  ...
  for (const template of fieldTemplates) {
    ...
    // Validate the value
    const validation = validateFieldData(template.fieldType, value)
    if (!validation.isValid && value !== null && value !== undefined && value !== '') {
      throw new Error(`Invalid value for field ${template.title} (${template.fieldType}): ${validation.error || value}`)
    }
