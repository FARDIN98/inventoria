 ...
 * @param {string} fieldType - Type of the field (from FIELD_TYPES)
 * @param {*} value - Value to validate
 * @returns {Promise<boolean>} True if valid, false otherwise
 */
export async function validateFieldData(fieldType, value) {
  if (!Object.values(FIELD_TYPES).includes(fieldType)) {
    throw new Error(`Invalid field type: ${fieldType}`)
  }

  // Null/undefined values are generally allowed (optional fields)
