...
export async function editInventoryAction(inventoryId, formData, currentVersion) {
  try {
    ...
    // Admins have full access to any inventory (public or private)
    // Owners have access to their own inventories
    // Regular users can only access public inventories
...
// Update inventory custom ID format with optimistic locking
export async function updateInventoryCustomIdFormatAction(inventoryId, customIdFormat, currentVersion) {
  try {
    const user = await getAuthenticatedUser()
    ...
    // Update inventory custom ID format with version increment
    const { data: updatedInventory, error: updateError } = await supabase
      .from('inventories')
      .update({
    ...
    if (updateError) {
      ...
      console.error('Database error updating inventory custom ID format:', updateError)
      return { success: false, error: `Failed to update custom ID format: ${updateError.message}` }
    }

    revalidatePath('/dashboard')
    revalidatePath(`/inventory/${inventoryId}`)
    revalidatePath(`/inventory/${inventoryId}/settings/custom-id`)
    return { success: true, inventory: updatedInventory }

  } catch (error) {
    console.error('Error updating inventory custom ID format:', error)
    return { success: false, error: `Failed to update custom ID format: ${error.message}` }
  }
}