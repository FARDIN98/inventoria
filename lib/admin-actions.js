...
export async function checkAdminPermission() {
  try {
    ...
    // Don't log authentication errors as they are expected for non-authenticated users
...
export async function bulkDeleteUsersAction(userIds) {
  try {
    ...
    const { error: transactionError } = await supabase.rpc('bulk_delete_users_with_items', {
      user_ids: userIds
    })
    
    if (transactionError) {
      console.error('Error bulk deleting users:', transactionError)
      return { success: false, error: `Failed to delete users: ${transactionError.message}` }
    }
